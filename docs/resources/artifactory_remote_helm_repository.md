...
* `external_dependencies_patterns` - (Optional) An Allow List of Ant-style path expressions that specify where external
  ...
  from any external source. Note that the official documentation states the default is '**', which is correct when creating
  repositories in the UI, but incorrect for the API. This provider behavior will match to UI.
