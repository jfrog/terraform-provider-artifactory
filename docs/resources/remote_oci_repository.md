...
* `enable_token_authentication` - (Optional, Default: `true`) Enable token (Bearer) based authentication. Unlike most remote package types (default `false`), OCI remotes default to `true`.
