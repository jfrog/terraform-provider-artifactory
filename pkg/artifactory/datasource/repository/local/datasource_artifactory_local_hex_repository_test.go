...
func TestAccLocalHexRepositoryDataSource_framework(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.artifactory_local_hex_repository."+name, "key", name),
					resource.TestCheckResourceAttr("data.artifactory_local_hex_repository."+name, "hex_primary_keypair_ref", kpName),
					resource.TestCheckResourceAttr("data.artifactory_local_hex_repository."+name, "repo_layout_ref", func() string {
						r, _ := repository.GetDefaultRepoLayoutRef("local", repository.HexPackageType)
						return r
					}()),
				),
