...
func TestAccDataSourceRemoteNimModelRepository(t *testing.T) {
	_, fqrn, name := testutil.MkNames("nimmodel-remote-test-repo-basic", "data.artifactory_remote_nimmodel_repository")

	cfg := util.ExecuteTemplate("nimmodel-remote-ds", `
		...
		data "artifactory_remote_nimmodel_repository" "{{ .repo_name }}" {
			key = artifactory_remote_nimmodel_repository.{{ .repo_name }}.key
		}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(fqrn, "package_type", "nimmodel"),
