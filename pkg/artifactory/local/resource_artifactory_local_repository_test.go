...
func TestAccLocalAlpineRepository(t *testing.T) {
	_, fqrn, name := util.MkNames("terraform-local-test-repo-basic", "artifactory_local_alpine_repository")
	kpId, kpFqrn, kpName := util.MkNames("some-keypair", "artifactory_keypair")
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: util.CompositeCheckDestroy(
			util.VerifyDeleted(fqrn, artifactory.testCheckRepo),
			util.VerifyDeleted(kpFqrn, security.VerifyKeyPair),
		),
...
func TestAccLocalDebianRepository(t *testing.T) {
	_, fqrn, name := util.MkNames("local-debian-repo", "artifactory_local_debian_repository")
	kpId, kpFqrn, kpName := util.MkNames("some-keypair1", "artifactory_keypair")
	kpId2, kpFqrn2, kpName2 := util.MkNames("some-keypair2", "artifactory_keypair")
	localRepositoryBasic := util.ExecuteTemplate("keypair", `
		resource "artifactory_keypair" "{{ .kp_name }}" {
			pair_name  = "{{ .kp_name }}"
			pair_type = "GPG"
	...
	resource.Test(t, resource.TestCase{
		PreCheck: func() { artifactory.testAccPreCheck(t) },
		CheckDestroy: artifactory.compositeCheckDestroy(
			util.verifyDeleted(fqrn, artifactory.testCheckRepo),
			util.verifyDeleted(kpFqrn, security.VerifyKeyPair),
			util.verifyDeleted(kpFqrn2, security.VerifyKeyPair),
		),
		ProviderFactories: artifactory.testAccProviders,
		Steps: []resource.TestStep{
			{
...
func TestAccLegacyLocalRepository_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		PreCheck:          func() { artifactory.testAccPreCheck(t) },
		CheckDestroy:      util.verifyDeleted(resourceName, artifactory.testCheckRepo),
		ProviderFactories: artifactory.testAccProviders,
		Steps: []resource.TestStep{
			{
				Config: localRepositoryBasic,
...
func TestAccLocalDockerV1Repository(t *testing.T) {
	...
	_, fqrn, name := util.MkNames("dockerv1-local", "artifactory_local_docker_v1_repository")
	...
	resource.Test(t, resource.TestCase{
		PreCheck:          func() { artifactory.testAccPreCheck(t) },
		CheckDestroy:      util.verifyDeleted(fqrn, artifactory.testCheckRepo),
		ProviderFactories: artifactory.testAccProviders,
		Steps: []resource.TestStep{
			{
				Config: localRepositoryBasic,
...
func TestAccLocalDockerV2Repository(t *testing.T) {
	...
	_, fqrn, name := util.MkNames("dockerv2-local", "artifactory_local_docker_v2_repository")
	params := map[string]interface{}{
		"block":     artifactory.randBool(),
		"retention": artifactory.randSelect(1, 5, 10),
		"max_tags":  artifactory.randSelect(0, 5, 10),
		"name":      name,
	}
	localRepositoryBasic := util.ExecuteTemplate("TestAccLocalDockerV2Repository", `
...
func TestAccLocalNugetRepository(t *testing.T) {
	...
	_, fqrn, name := util.MkNames("nuget-local", "artifactory_local_nuget_repository")
	params := map[string]interface{}{
		"force_nuget_authentication": artifactory.randBool(),
		"max_unique_snapshots":       artifactory.randSelect(0, 5, 10),
		"name":                       name,
	}
...
func TestAccLocalMavenRepository(t *testing.T) {
	...
	_, fqrn, name := util.MkNames("maven-local", "artifactory_local_maven_repository")
	tempStruct := make(map[string]interface{})
	artifactory.copyInterfaceMap(commonJavaParams, tempStruct)
...
func TestAccLocalGradleRepository(t *testing.T) {
	...
	_, fqrn, name := util.MkNames("gradle-local", "artifactory_local_gradle_repository")
	tempStruct := make(map[string]interface{})
	artifactory.copyInterfaceMap(commonJavaParams, tempStruct)
...
func TestAccLocalGenericRepository(t *testing.T) {
	...
	_, fqrn, name := util.MkNames("generic-local", "artifactory_local_generic_repository")
	params := map[string]interface{}{
		"name": name,
	}
...
func TestAccLocalNpmRepository(t *testing.T) {
	...
	_, fqrn, name := util.MkNames("npm-local", "artifactory_local_npm_repository")
	params := map[string]interface{}{
		"name": name,
	}
...
func TestAccAllLegacyRepoTypesLocal(t *testing.T) {
	...
	for _, repo := range artifactory.repoTypesLikeGeneric {
