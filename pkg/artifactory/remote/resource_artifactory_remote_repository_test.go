...
func TestAccLocalAllowDotsUnderscorersAndDashesInKeyGH129(t *testing.T) {
	_, fqrn, name := util.MkNames("terraform-local-test-repo-basic", "artifactory_remote_repository")

	key := fmt.Sprintf("debian-remote.teleport_%d", util.randomInt())
...
func TestAccRemoteRepositoryChangeConfigGH148(t *testing.T) {
	_, fqrn, name := util.MkNames("github-remote", "artifactory_remote_repository")
	const step1 = `
		locals {
...
func mkNewRemoteTestCase(repoType string, t *testing.T, extraFields map[string]interface{}) (*testing.T, resource.TestCase) {
	_, fqrn, name := util.MkNames("terraform-remote-test-repo-full", fmt.Sprintf("artifactory_remote_%s_repository", repoType))
...
%s
	...
	`
	extraChecks := util.mapToTestChecks(fqrn, extraFields)
	defaultChecks := util.mapToTestChecks(fqrn, allFields)
...
func mkLegacyRemoteTestCase(repoType string, t *testing.T) (*testing.T, resource.TestCase) {
	...
	_, fqrn, name := util.MkNames("terraform-remote-test-repo-full", "artifactory_remote_repository")
	return t, resource.TestCase{
		ProviderFactories: artifactory.TestAccProviders,
		PreCheck:          func() { artifactory.testAccPreCheck(t) },
		CheckDestroy:      util.verifyDeleted(fqrn, util.testCheckRepo),
