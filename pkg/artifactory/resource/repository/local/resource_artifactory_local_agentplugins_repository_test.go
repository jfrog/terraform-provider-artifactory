...
func TestAccLocalAgentPluginsRepository(t *testing.T) {
	acctest.SkipIfNotSupportedVersion(t, "7.155.3")

	_, fqrn, name := testutil.MkNames("agentplugins-local", "artifactory_local_agentplugins_repository")

	temp := `
	resource "artifactory_local_agentplugins_repository" "{{ .name }}" {
		key                      = "{{ .name }}"
	...
	config := util.ExecuteTemplate("TestAccLocalAgentPluginsRepository", temp, params)
	...
	updatedConfig := util.ExecuteTemplate("TestAccLocalAgentPluginsRepository", temp, updatedParams)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(fqrn, "repo_layout_ref", func() string {
						r, _ := repository.GetDefaultRepoLayoutRef("local", repository.AgentPluginsPackageType)
						return r
