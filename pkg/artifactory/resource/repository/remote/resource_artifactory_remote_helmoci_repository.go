...
var HelmOciRemoteSchema = func(isResource bool) map[string]*schema.Schema {
...
func ResourceArtifactoryRemoteHelmOciRepository() *schema.Resource {
	var unpackRemoteRepo = func(s *schema.ResourceData) (interface{}, string, error) {
		...
		repo := HelmOciRemoteRepo{
			RepositoryRemoteBaseParams:   UnpackBaseRemoteRepo(s, HelmOciPackageType),
	...
	schema := HelmOciRemoteSchema(true)
	...
	return mkResourceSchema(schema, remoteRepoPacker, unpackRemoteRepo, constructor)
}
