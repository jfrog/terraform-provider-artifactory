...
func resourceArtifactoryFederatedRepository() *schema.Resource {
	return &schema.Resource{
		Create: resourceFederatedRepositoryCreate,
		Read:   resourceFederatedRepositoryRead,
		Update: resourceFederatedRepositoryUpdate,
		Delete: resourceFederatedRepositoryDelete,
		Exists: resourceFederatedRepositoryExists,
...
func unmarshalFederatedRepository(s *schema.ResourceData) *v1.FederatedRepository {
	...
	repo := new(v1.FederatedRepository)

	repo.RClass = artifactory.String("federated")
...
func resourceFederatedRepositoryCreate(d *schema.ResourceData, m interface{}) error {
	...
	d.SetId(*repo.Key)
	return resourceFederatedRepositoryRead(d, m)
}
...
func resourceFederatedRepositoryUpdate(d *schema.ResourceData, m interface{}) error {
	c := m.(*ArtClient).ArtOld

	repo := unmarshalFederatedRepository(d)
	_, err := c.V1.Repositories.UpdateFederated(context.Background(), d.Id(), repo)
...
func resourceFederatedRepositoryDelete(d *schema.ResourceData, m interface{}) error {
	c := m.(*ArtClient).ArtOld
	repo := unmarshalFederatedRepository(d)

	resp, err := c.V1.Repositories.DeleteFederated(context.Background(), *repo.Key)
...
func resourceFederatedRepositoryExists(d *schema.ResourceData, m interface{}) (bool, error) {
	c := m.(*ArtClient).ArtOld

	_, resp, err := c.V1.Repositories.GetFederated(context.Background(), d.Id())
