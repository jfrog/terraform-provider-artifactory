...
func resourceWatchCreate(d *schema.ResourceData, m interface{}) error {
	client := m.(*ArtClient).JfrogXray

	if client == nil {
		return fmt.Errorf("jfrog xray client is nil. please configure xray settings in provider")
	}
...
func resourceWatchRead(d *schema.ResourceData, m interface{}) error {
	...
	watch, err := client.GetWatch(d.Id())
...
func unpackWatch(d *schema.ResourceData, m interface{}) (*xrayutils.WatchParams, error) {
	...
	repositoriesRaw := d.Get("repository").(*schema.Set).List()
	allBuildsRaw := d.Get("all_builds").([]interface{})
	buildsRaw := d.Get("build").(*schema.Set).List()
...
func unpackRepository(repositories []interface{}, artClient *artifactoryold.Artifactory, params *xrayutils.WatchParams) error {

	if len(repositories) > 0 {

		params.Repositories.Type = xrayutils.WatchRepositoriesByName
		// There can be 1 to M repositories
		params.Repositories.Repositories = make(map[string]xrayutils.WatchRepository)

		for _, repositoryRaw := range repositories {
...
func unpackBuild(builds []interface{}, params *xrayutils.WatchParams) {

	if len(builds) > 0 {
		params.Builds.Type = xrayutils.WatchBuildByName
		params.Builds.ByNames = make(map[string]xrayutils.WatchBuildsByNameParams)

		for _, buildRaw := range builds {
