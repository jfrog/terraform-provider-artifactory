...
func ResourceArtifactoryPermissionTarget() *schema.Resource {
	...
	buildSchema.Elem.(*schema.Resource).Schema["repositories"].Description = `This can only be 1 value: "artifactory-build-info", and currently, validators of sets/lists is not allowed. Artifactory will reject the request if you change this`
...
func resourcePermissionTargetCreate(d *schema.ResourceData, m interface{}) error {
	...
	if _, err := m.(*resty.Client).R().AddRetryCondition(retry.On400Error).SetBody(permissionTarget).Post(permissionsEndPoint + permissionTarget.Name); err != nil {
		return err
	}
