...
func expandPolicy(d *schema.ResourceData) (*Policy, error) {
	...
	policy.Name = util.StringPtr(d.Get("name").(string))
	if v, ok := d.GetOk("type"); ok {
		policy.Type = util.StringPtr(v.(string))
	}
	if v, ok := d.GetOk("description"); ok {
		policy.Description = util.StringPtr(v.(string))
	}
	if v, ok := d.GetOk("author"); ok {
		policy.Author = util.StringPtr(v.(string))
	}
...
func expandRules(configured []interface{}, policyType *string) (policyRules []PolicyRule, err error) {
	...
	for _, raw := range configured {
		...
		rule.Name = util.StringPtr(data["name"].(string))
		rule.Priority = util.IntPtr(data["priority"].(int))
...
func expandCriteria(l []interface{}, policyType *string) (*PolicyRuleCriteria, error) {
	...
	m := l[0].(map[string]interface{}) // We made this a list of one to make schema validators easier
	...
	minSev := util.StringPtr(m["min_severity"].(string))
	cvss := expandCVSSRange(m["cvss_range"].([]interface{}))
	allowUnk := util.BoolPtr(m["allow_unknown"].(bool))
...
func expandCVSSRange(l []interface{}) *PolicyCVSSRange {
	...
	cvssrange := &PolicyCVSSRange{
		From: util.IntPtr(m["from"].(int)),
		To:   util.IntPtr(m["to"].(int)),
	}
	return cvssrange
}
...
func expandActions(l []interface{}) *PolicyRuleActions {
	...
	if v, ok := m["fail_build"]; ok {
		actions.FailBuild = util.BoolPtr(v.(bool))
	}
	...
	if v, ok := m["block_download"]; ok {
		if len(v.([]interface{})) > 0 {
			...
			actions.BlockDownload = &BlockDownloadSettings{
				Unscanned: util.BoolPtr(vMap["unscanned"].(bool)),
				Active:    util.BoolPtr(vMap["active"].(bool)),
			}
		} else {
			actions.BlockDownload = &BlockDownloadSettings{
				Unscanned: util.BoolPtr(false),
				Active:    util.BoolPtr(false),
			}
