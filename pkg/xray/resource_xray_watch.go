...
func expandWatch(d *schema.ResourceData) *Watch {
	...
	gd := &WatchGeneralData{
		Name: util.StringPtr(d.Get("name").(string)),
	}
	if v, ok := d.GetOk("description"); ok {
		gd.Description = util.StringPtr(v.(string))
	}
	if v, ok := d.GetOk("active"); ok {
		gd.Active = util.BoolPtr(v.(bool))
	}
...
func expandProjectResource(rawCfg interface{}) *WatchProjectResource {
	...
	resource.Type = util.StringPtr(cfg["type"].(string))
	if v, ok := cfg["bin_mgr_id"]; ok {
		resource.BinaryManagerId = util.StringPtr(v.(string))
	}
	if v, ok := cfg["repo_type"]; ok {
		resource.RepoType = util.StringPtr(v.(string))
	}
	if v, ok := cfg["name"]; ok {
		resource.Name = util.StringPtr(v.(string))
	}
...
func expandFilters(l []interface{}) []WatchFilter {
	...
	for _, raw := range l {
		...
		filter.Type = util.StringPtr(f["type"].(string))
		valueWrapper := new(WatchFilterValueWrapper)
		fv := new(WatchFilterValue)
		fv.Value = util.StringPtr(f["value"].(string))
...
func expandAssignedPolicy(rawCfg interface{}) *WatchAssignedPolicy {
	...
	cfg := rawCfg.(map[string]interface{})
	policy.Name = util.StringPtr(cfg["name"].(string))
	policy.Type = util.StringPtr(cfg["type"].(string))
